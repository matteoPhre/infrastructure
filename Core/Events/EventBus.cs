...
namespace Infrastructure.Core.Events
{
    public class EventBus : IEventBus
    {
        ...
        private async Task SendToMessageBroker(IEvent @event)
        {
            ...
            {
                await _eventListener.PublishMessage(new Message<IEvent>(@event));
            }
            else
            {
                throw new ArgumentNullException("No event listener found");
        ...
        private async Task SendToMessageBroker(IMessage @event)
        {
            if (_outboxListener != null)
            {
                await _outboxListener.CommitMessage(@event);
            }
            else if (_eventListener != null)
            {
                await _eventListener.PublishMessage(@event);
            }
            else
            {
                throw new ArgumentNullException("No event listener found");
        ...
        public async Task PublishLocal(params IMessage[] events)
        {
            foreach (var @event in events)
            {
                await _mediator.Publish(@event);
        ...
        public async Task Commit(params IMessage[] events)
        {
            foreach (var @event in events)
            {
                await SendToMessageBroker(@event);
